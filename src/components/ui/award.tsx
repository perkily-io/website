...
export function Awards({
  ...
  if (variant === "badge") {
    const Badge = () => (
      <svg
        className={cn("fill-yellow-400 h-8 w-8")}
        width="32"
        height="32"
        viewBox="0 0 32 32"
      >
        <path d="M16 2L18.09 8.26L24 8L19 12.74L20.18 18.02L16 15L11.82 18.02L13 12.74L8 8L13.91 8.26L16 2Z" />
      </svg>
    )
    return (
      <div className={cn("", className)}>
        <div
          className={cn("rounded-lg border border-white/20 p-3", "flex items-center gap-3 bg-white/5 backdrop-blur-sm")}
        >
          <Badge />
          <div>
            <h1 className={cn("text-white text-lg font-semibold tracking-tight")}>
              {title}
            </h1>
            <h2 className={cn("text-white/60 text-sm font-light")}>
              {subtitle}
            </h2>
